interface KnapsackReturn {
  [index: number] : number[] | number
}

function knapsackProblem(items: number[][], capacity: number): KnapsackReturn {
  //rows - items
  ...
  return [getKnapSackItems(knapSackValues, items), knapSackValues[knapSackValues.length - 1][knapSackValues[0].length - 1]];
}
